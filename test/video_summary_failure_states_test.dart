...
void main() {
  group('AI summary failure mapping', () {
    test('explicit failure states keep their own messages without fallback', () {
      expect(
    ...
    test('default messages stay distinct for each failure class', () {
      ...
      expect(
        AiConclusionPanel.messageForResult(const AiSummaryServiceMisconfigured()),
        '请先完成 AI 总结服务配置',
      );
      expect(
        AiConclusionPanel.messageForResult(const AiSummaryServiceProviderError()),
        'AI 总结生成失败',
      );
      expect(
    ...
    test('success without usable content does not masquerade as a failure type', () {
      expect(
        AiConclusionPanel.messageForResult(
